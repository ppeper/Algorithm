...
class SecretMap {
    public String[] solution(int n, int[] arr1, int[] arr2) {
        ...
        String[] str1 = getBinary(arr1, n);
        String[] str2 = getBinary(arr2, n);

        for (int i = 0; i < answer.length; i++) {
            answer[i] = getResult(str1[i], str2[i]);
        }
        return answer;
    }

    // 2진수 문자열배열로 반환 -> 배열크기에 맞추어주기
    public String[] getBinary(int[] arr,int n) {
        String[] strings = new String[arr.length];
        for (int i = 0; i < arr.length; i++) {
            strings[i] = String.format("%0"+n+"d",Long.parseLong(Integer.toBinaryString(arr[i])));
        }
        return strings;
    }
